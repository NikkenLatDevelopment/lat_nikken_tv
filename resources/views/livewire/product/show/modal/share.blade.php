<div>
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="input-group rounded-3 overflow-hidden mb-3">
                        <div class="form-floating">
                            <input type="text" class="form-control fw-bold text-decoration-underline bg-light border-end-0 border-light shadow-none" id="product-show-modal-share-url" placeholder="Link" value="{{ $currentUrl }}">
                            <label for="product-show-modal-share-url">Link</label>
                        </div>

                        <button type="button" class="btn btn-sm input-group-text link-success fw-bold bg-light border-light pe-3" id="product-show-modal-share-btn"><i class="fi fi-rr-copy-alt fs-5 position-relative mb-0 me-1 custom i-top-3"></i> Copiar</button>
                    </div>

                    @if (!auth()->check())
                        <p class="small"><span class="fw-bold">Nota:</span> Si cuentas con tu Tienda Virtual Personalizada, <a href="{{ route('login') }}" class="text-success fw-bold">inicia sesión</a> para obtener tu link personal.</p>
                    @endif
    ...
    @script
        ...
        @push('scripts')
            <script>
                document.getElementById("product-show-modal-share-btn").addEventListener("click", function () {
                    let copyText = document.getElementById("product-show-modal-share-url");
                    copyText.select();
                ...
                document.getElementById("product-show-modal-share-url").addEventListener("click", function () {
                    this.focus();
                    this.select();
                });
            </script>
